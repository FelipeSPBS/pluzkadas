...
import uuid
import requests
from constants import APIkey, plano4, groupId, plano1, plano2, plano3, PUSHINPAY_TOKEN, plano1DESC, plano2DESC, plano3DESC, pack1, pack2, pack3, Orderbump1, Orderbump2, Orderbump3
...
def generate_invite_link():
    return "https://t.me/+FHxfpW2aunVhMzVh"  # Altere pelo seu link real
...
def start(msg):
    ...
    with open("ph-apresenta.mp4", "rb") as video:  # Use nosso vídeo reencodado
        bot.send_video(chat_id, video)  

    bot.send_message(chat_id, 
                    "😈⚡️🔥 Tenha acesso a milhares de conteúdos vazados e completos em um só Lugar.\n\n"
                    "🟢 VALOR:⤵️\n\n"
                    "▪️ R$ 19,90/ PAGAMENTO ÚNICO\n\n"
                    ...
                    "TUDO ISSO E MUITO MAIS, POR APENAS R$ 19,90😱 (preço de pinga )\n\n"
...
def welcome_user(chat_id):
    with open("ph-apresenta-reencodado.mp4", "rb") as video:  # Use nosso vídeo reencodado
        bot.send_video(chat_id, video)  
...
def sendPromo(chat_id):
    ...
    try:
        ...
        bot.send_message(
            chat_id,
"Oi, amor… 💋\n"
"Não pude deixar de notar que você queria entrar no meu VIP… 👀 Então decidi fazer algo irresistível pra você:\n\n"

"🔥 Liberado hoje: VIP Vitalício + BÔNUS de 39,90 por só R$24,90!\n"
"💸 Pagamento único — acesso eterno, sem mensalidade!\n\n"

"Mas é só pra quem agir agora… Depois eu fecho o acesso. 😈💖\n\n"

"Vai ficar só olhando de novo?\n",
...
def PROMO():
    while True:
        try:
            ...
            time.sleep(25200)  # Espera 10 minutos (600 segundos) antes de começar de novo

        except Exception as e:
            print(f'Erro ao enviar promoções: {e}')
            time.sleep(21600)  # Espera 1 minuto antes de tentar novamente
...
def restart_bot():
    ...
    # O comando abaixo é usado para reiniciar o script
    subprocess.Popen([sys.executable] + sys.argv)  # Inicia um novo processo
    sys.exit()  # Encerra o processo atual

def restart_every_three_hours():
    while True:
        time.sleep(600)  # Aguardar 3 horas (10800 segundos)
        restart_bot()
